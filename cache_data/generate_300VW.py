...
def load_video_dir(root, dirs, save_dir, save_name):
  ...
  for idx, cdir in enumerate(dirs):
    ...
    input_seq_length = 10
    padding_num_frames = input_seq_length - len(all_frames) % input_seq_length
    padding_num_annots = input_seq_length - len(all_frames) % input_seq_length
    if padding_num_frames!=0:
        pad = [all_frames[i] for i in range(-1-padding_num_frames, -1)]
        pad.reverse()
        all_frames = all_frames + pad
    if padding_num_annots!=0:
        pad = [all_annots[i] for i in range(-1-padding_num_annots, -1)]
        pad.reverse()
        all_annots = all_annots + pad
...
if __name__ == '__main__':
  ...
  path_300vw = osp.join('/data1/mry', 'datasets', 'landmark-datasets', '300VW_Dataset_2015_12_14')
