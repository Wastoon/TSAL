...
def main(args):
    ...
    if args.Rma_model:
        ...
        if hasattr(student_net, 'specify_parameter'):
            ...
            Student_net_param_dict = student_net.parameters()

        # parameter of Student_MFEM
        Student_MFEM_param_dict = MFEM_student.parameters()
        Student_optimizer_MFEM, Student_scheduler_MFEM, Student_criterion_MFEM = obtain_optimizer(Student_MFEM_param_dict, opt_config, logger)
        Student_optimizer, Student_scheduler, Student_criterion = obtain_optimizer(Student_net_param_dict, opt_config, logger)
    #if args.usefocalloss:
    #    criterion = FocalLoss()
    logger.log('criterion : {:}'.format(criterion))
    if args.Rma_model:
        logger.log('Student_criterion : {:}'.format(Student_criterion))
    ...
    if args.Rma_model:
        student_net, Student_criterion = student_net.cuda(), Student_criterion.cuda()
        student_net = torch.nn.DataParallel(student_net)
        MFEM_student = MFEM_student.cuda()
        MFEM_student = torch.nn.DataParallel(MFEM_student)
