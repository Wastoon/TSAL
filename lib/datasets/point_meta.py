...
class Point_Meta():
    ...
    def __init__(self, num_point, points, box, image_path, dataset_name):
        ...
        if points is None:
            ...
            assert len(points.shape) == 2 and points.shape[0] == 3 and points.shape[
                1] == self.num_point, 'The shape of point is not right : {}'.format(points)
    ...
    def apply_bound(self, width, height):
        if self.points is not None:
            oks = np.vstack((self.points[0, :] >= 0, self.points[1, :] >= 0, self.points[0, :] <= width,
                             self.points[1, :] <= height, self.points[2, :].astype('bool')))
    ...
    def apply_rotate(self, center, degree):
        ...
        if self.points is not None:
            ...
            self.points[0, self.points[2, :] > 0] = (vis_xs - center[0]) * np.cos(degree) - (
                        vis_ys - center[1]) * np.sin(degree) + center[0]
            self.points[1, self.points[2, :] > 0] = (vis_xs - center[0]) * np.sin(degree) + (
                        vis_ys - center[1]) * np.cos(degree) + center[1]
        # rotate the box
        ...
        corners[:, 0] = (corners[:, 0] - center[0]) * np.cos(degree) - (corners[:, 1] - center[1]) * np.sin(degree) + \
                        center[0]
        corners[:, 1] = (corners[:, 0] - center[0]) * np.sin(degree) - (corners[:, 1] - center[1]) * np.cos(degree) + \
                        center[1]
