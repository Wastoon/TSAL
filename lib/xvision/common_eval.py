...
def evaluate_normalized_mean_error(predictions, groundtruth, log, extra_faces):
    ...
    for i in range(num_images):
        ...
        if num_points == 68:
            ...
            # IPN
            left_center = np.sum(ground_truth_points[:2, 36:42], axis=1).reshape(2, 1) / 6
            right_center = np.sum(ground_truth_points[:2, 42:48], axis=1).reshape(2, 1) / 6
            interocular_distance = np.linalg.norm(left_center - right_center)
